    ...
    if st.button('Run Simulation'):
        try:
            with st.spinner(f'Fetching historical data for {ticker}...'):
                ...
                if len(data) < 30:
                    st.warning(f"Limited data available for {ticker}. Results may be less reliable.")
                ...
                if sigma == 0:
                    st.error("Standard deviation is zero. Cannot run simulation.")
                    return
                    
                if np.isnan(mu) or np.isnan(sigma):
                    st.error("Invalid statistical parameters. Please check the data quality.")
                    return
                ...
                use_vectorized = st.checkbox("Use Vectorized Simulation (Faster)", value=True)
                
                if use_vectorized:
                    # VECTORIZED APPROACH
                    with st.spinner('Running vectorized Monte Carlo simulation...'):
                        # Generate all random numbers at once
                        Z = np.random.normal(0, 1, size=(num_simulations, days-1))
                        
                        # Calculate log returns for all paths
                        log_returns_sim = (mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * Z
                        
                        # Calculate cumulative log returns
                        cumulative_returns = np.cumsum(log_returns_sim, axis=1)
                        
                        # Calculate all price paths at once
                        price_paths = S0 * np.exp(np.column_stack([np.zeros(num_simulations), cumulative_returns]))
                        
                        # Convert to DataFrame
                        simulation_df = pd.DataFrame(price_paths.T, 
                                                    columns=[f'Sim_{i}' for i in range(num_simulations)])
                    
                    st.success("Vectorized simulation completed!")
                    
                else:
                    # FIXED LOOP-BASED APPROACH
                    ...
                    for i in range(num_simulations):
                        ...
                        # Changed from size=(days) to size=(days-1)
                        Z = np.random.normal(0, 1, size=(days-1))
                        ...
                        # Changed Z[t] to Z[t-1] to match the array size
                ...
                # Set seed for consistent subset selection
                np.random.seed(123)
